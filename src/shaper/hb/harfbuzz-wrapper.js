import createHarfBuzz from "./harfbuzz-glue.js";
...
let __hbReady = false;
/**
 * Initialize the vendored HarfBuzz module from raw wasm bytes. Must be awaited
 * once before any Blob/Face/Font/Buffer/shape call. Idempotent.
 */
async function initHarfBuzz(wasmBinary) {
	if (__hbReady) return;
	init(await createHarfBuzz({ wasmBinary }));
	__hbReady = true;
}
function harfBuzzReady() { return __hbReady; }
export { initHarfBuzz, harfBuzzReady, Blob, Buffer, BufferContentType, BufferFlag, BufferSerializeFlag, BufferSerializeFormat, ClusterLevel, Direction, Face, Feature, Font, FontFuncs, GlyphClass, GlyphFlag, MetricsTag, TracePhase, Variation, otTagToLanguage, otTagToScript, shape, shapeWithTrace, version, versionString };
