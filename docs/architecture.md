...
Everything else is pure enough to run under `node --test`, which is why 128 unit
...
written from a shaped glyph's `xOffset` otherwise), the packed foreground, and a
style word: bit 0 colored, bit 1 faint, bit 2 blink, bits 8 to 15 the atlas page. Decorations are two
...
Shaping satisfies the invariant too, which is not obvious given that it moves
characters between columns. It moves them within the row: a shaped glyph's
`xOffset` is an offset from its own column's left edge, and the run grouping that
produced it is planned per row from the row's own cells. A slot carrying a shaped
run therefore draws the same picture wherever the rotation puts it, and the same
equivalence cases are run a second time with `arabicShaper` configured to hold
that claim to pixels.
